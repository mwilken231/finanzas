...
import { getPlans, createPlan, updatePlan, deletePlan, getCategories } from '../api'
import { formatARS, currentMonth } from '../utils'
...
function PlanRow({ plan, onEdit, onDelete, onSaveItems }) {
  ...
  return (
    <div>
      <div className="px-4 py-3 flex items-center gap-2 hover:bg-slate-700/20 group">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="text-sm font-medium text-slate-200">{plan.categoria?.nombre || plan.detalle || 'Sin categoría'}</span>
        ...
        <span className="font-semibold text-emerald-400 text-sm">{formatARS(plan.monto_planificado)}</span>
      ...
      {open && (
        <SubItemsEditor items={items} planId={plan.id} onSave={onSaveItems} />
      )}
    </div>
  )
}
...
export default function IncomePlan() {
  ...
  const [form, setForm] = useState({ categoria_id: '', label: '', monto_planificado: '', es_recurrente: false, items: [] })

  const load = useCallback(async () => {
    setLoading(true)
    try {
      const [p, cats] = await Promise.all([getPlans(mes, 'ingreso'), getCategories('ingreso')])
  ...
  const openEdit = (plan) => {
    ...
    setForm({
      ...
      label: plan.detalle && !plan.detalle.startsWith('{') ? plan.detalle : '',
  ...
  const openNew = () => {
    setEditPlan(null)
    setForm({ categoria_id: '', label: '', monto_planificado: '', es_recurrente: false, items: [] })
    setModal('form')
  }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const detalle = form.items.length > 0
        ? JSON.stringify({ items: form.items })
        : (form.label || null)

      const data = {
        mes: `${year}-${month}-01`, tipo: 'ingreso',
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <h2 className="text-xl font-bold text-white">Plan de Ingresos</h2>
      ...
      {loading ? (
        ...
        <div className="card divide-y divide-slate-700/50 p-0 overflow-hidden">
          {plans.length === 0 ? (
            <p className="px-5 py-10 text-center text-slate-500">Sin plan de ingresos para este período</p>
          ) : plans.map((p) => (
          ...
          <div className="px-4 py-3 flex justify-between font-semibold">
            <span className="text-slate-300">Total proyectado</span>
            <span className="text-emerald-400">{formatARS(total)}</span>
          </div>
        </div>
      )}

      <Modal open={modal === 'form'} onClose={() => { setModal(null); setEditPlan(null) }} title={editPlan ? 'Editar ingreso' : 'Nuevo ingreso proyectado'}>
        <div className="space-y-4">
          {!editPlan && (
            <div>
          ...
          <div>
            ...
            <input
              className="input"
              placeholder="Ej: Salario, Alquiler casa..."
              value={form.label}
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={addFormItem} className="text-xs text-emerald-400 hover:text-emerald-300">+ Agregar</button>
            </div>
            {form.items.length > 0 ? (
              <div className="border border-slate-700 rounded-lg overflow-hidden divide-y divide-slate-700/60">
                ...
                <div className="flex justify-between px-3 py-1.5 bg-slate-900/40">
                  ...
                  <span className="text-xs font-semibold text-emerald-400">{formatARS(itemTotal)}</span>
                </div>
              </div>
            ) : (
              <div>
                <label className="label">Monto proyectado</label>
